...
**当前对外汇报成绩:dev br@80 = 0.5417**(放行 80% 合格视频时拦下 54.2% 的问题视频)。
...
## 3. 内部口径备注:图片探针的隔离(对外汇报一律用 0.5417,本节仅供复现时对齐数字)
...
**上线不受影响**(新源图不在训练池),受影响的是自估。两套数字都留档;**对外汇报与周报一律按 0.5417 口径**,隔离口径仅用于内部对齐与后续迭代。
