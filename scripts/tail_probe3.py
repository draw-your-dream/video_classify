...
ROOT = Path("/root/mech")
Q = """以下图片是同一条AI生成视频的逐帧画面(已放大到角色身体下半部分,按时间顺序)。主角是毛绒蘑菇角色「蘑菇TUTU」(浅黄色身体、红色伞盖)。
...
def lower_zoom(rel):
    ...
    for p in sorted(d.glob("f*.jpg"))[:16]:
        ...
        crop = im[int(H*0.40):, :]
        crop = cv2.resize(crop, (W*2, int((H-int(H*0.40))*2)), interpolation=cv2.INTER_CUBIC)
...
for tag, rel in (("目标5822", "基础款/5822.mp4"), ("负对照7060", "基础款/7060.mp4"), ("负对照6472", "花花款/6472.mp4")):
    j = parse(ask(lower_zoom(rel), Q))
    print(f"{tag}: {json.dumps(j, ensure_ascii=False)[:220]}", flush=True)
print("TAIL_PROBE3_DONE", flush=True)
