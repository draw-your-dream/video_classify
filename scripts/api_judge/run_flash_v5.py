...
RP.RUBRIC=open(Path(__file__).parent/"rubric_v5.txt",encoding="utf-8").read()
S=json.loads(json.dumps(RP.SCHEMA))
S["properties"]["style"]={"type":"string","enum":["cg","live","unclear"]}
S["properties"]["dim_scores"]={"type":"object","properties":{
    "ip":{"type":"integer","minimum":0,"maximum":100},
    ...
    "required":["ip","motion","interaction","physics","stability"]}
S["required"]=S["required"]+["style","dim_scores"]
RP.SCHEMA=S
...
def work(fn):
    vp=VID/fn; row={"filename":fn,"model":model,"rubric":"v5"}
    rp=REFS/(fn[:-4]+".png")
    if not rp.exists():
