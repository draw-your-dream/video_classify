...
V5={}
for l in open(OUT/"flash_v5_raw.jsonl"):
...
DIMS=["ip","motion","interaction","physics","stability"]
DS=np.zeros((N,5))
for i,v in enumerate(vids):
    d=(V5.get(v,{}).get("dim_scores") or {})
    DS[i]=[d.get(k,0) for k in DIMS]
style=np.array([V5.get(v,{}).get("style","unclear") for v in vids])
grade5=np.array([V5.get(v,{}).get("grade","") for v in vids])
print("  风格分布",dict(collections.Counter(style)),"判档",dict(collections.Counter(grade5)))
...
for j,k in enumerate(DIMS):
    print(f"    维度 {k:12s} 单独 br@80={br_at(rp(DS[:,j])[dm],y[dm]):.4f} AUC={auc(DS[dm,j],y[dm]):.4f}")
...
CFG=[("现配:两遍",[A2,trkpct(A2)],()),
     ("v5 替换两遍",[R5,trkpct(R5)],()),
     ("两遍+v5总分",[A2,trkpct(A2),R5],()),
     ("两遍+v5五维度",[A2,trkpct(A2)],[rp(DS[:,j]) for j in range(5)]),
     ("两遍+v5总分+五维度",[A2,trkpct(A2),R5],[rp(DS[:,j]) for j in range(5)])]
print("\n=== 融合层 ===")
for nm,fc,ex in CFG:
