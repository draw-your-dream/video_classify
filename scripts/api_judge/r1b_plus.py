...
外折并行(fork 共享特征矩阵)。输出:data/pbase/out/r1_oof.npz + r1_plus_stacks.pkl
"""
...
def train_three(X, y, seed=42):
    ...
    lr = make_pipeline(_SS(), SGDClassifier(loss="log_loss", alpha=1e-4, max_iter=60,
...
def run_fold(args_):
    ...
    for j, spec in enumerate(EXPERTS):
        ...
        oof_tr[:, j] = oof_j
        print(f"fold{k} expert{j} done", flush=True)
...
def main():
    ...
    # EXT:新信号折叠进专家与 R1c
    import csv as _csv
    _idx = {Path(v).name: i for i, v in enumerate(vids)}
    _ext_list = []
    for _fn in ["newref_feats.csv", "subjcons_1233.csv", "prop_timeline.csv", "newref2_1233.csv"]:
        _rows = list(_csv.DictReader(open(OUT / _fn)))
        def _isnum(x):
            try:
                float(x or 0); return True
            except ValueError:
                return False
        _keys = [k for k in _rows[0] if k not in ("filename", "label") and _isnum(_rows[0][k])]
        _M = np.zeros((len(vids), len(_keys)))
        for _r in _rows:
            if _r["filename"] in _idx:
                _M[_idx[_r["filename"]]] = [float(_r[k] or 0) for k in _keys]
        _ext_list.append(_M)
    EXT = np.hstack(_ext_list)
    print(f"EXT: {EXT.shape}", flush=True)
    for _nm in list(Xs):
        Xs[_nm] = np.hstack([Xs[_nm], EXT])
    X303 = np.hstack([X303, EXT])
    ...
    fold_args = [(k, tr, te) for k, (tr, te) in enumerate(outer.split(X303, y, groups))]
    from multiprocessing.dummy import Pool as TPool
    stacks = {}
    with TPool(10) as pool:
    ...
    pickle.dump(stacks, open(OUT / "r1_plus_stacks.pkl", "wb"))

    assert not np.isnan(r1b).any() and not np.isnan(r1c).any()
    ...
    np.savez(OUT / "r1_plus_oof.npz", r1b=r1b, r1c=r1c, y=y,
