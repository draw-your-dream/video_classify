...
V5={}
for l in open(OUT/"flash_v9_raw.jsonl"):
...
b8=np.array([v8.get(v,np.nan) for v in vids],float)
b8=np.where(np.isnan(b8),np.nanmedian(b8),b8); R8=rp(b8)
print(f"  v8(同配置对照)单独 dev br@80={br_at(R8[dm],y[dm]):.4f} AUC={auc(b8[dm],y[dm]):.4f}")
print(f"  秩相关 v9-v8 {spearmanr(b5,b8).statistic:.3f}")
A3=(rp(f1)+rp(f2)+R5)/3
CFG=[("现配:两遍",[A2,trkpct(A2)],()),
     ("v9 替换两遍",[R5,trkpct(R5)],()),
     ("两遍+v9列",[A2,trkpct(A2),R5],()),
     ("三遍均值(含v9)",[A3,trkpct(A3)],()),
     ("两遍+v8列(对照)",[A2,trkpct(A2),R8],()),
     ("两遍+v8+v9",[A2,trkpct(A2),R8,R5],())]
# 150 条重叠、同配置正面对撞
old={}
for l in open(Path.home()/"tutu-video-eval/data/out_holdout_full.jsonl"):
    o=json.loads(l); r=o.get("result") or {}
    if "bad_score" in r: old[o["filename"]]=(r["bad_score"], o.get("ref"))
com=[v for v in vids if v in old]
if com:
    ii=[idx[v] for v in com]
    a=np.array([old[v][0] for v in com],float); b=b5[ii]; yy=y[ii]
    same_ref=sum(1 for v in com if old[v][1]=="exact")
    print(f"\n=== 同配置对撞({len(com)} 条重叠,历史 exact {same_ref}) ===")
    print(f"  bad_score 均值 历史 {a.mean():.1f} vs v8 {b.mean():.1f};秩相关 {spearmanr(a,b).statistic:.3f}")
    print(f"  AUC 历史 {auc(a,yy):.4f} vs v8 {auc(b,yy):.4f}")
print("\n=== 融合层 ===")
for nm,fc,ex in CFG:
