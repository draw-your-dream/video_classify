...
V5={}
for l in open(OUT/"flash_v7_raw.jsonl"):
...
CFG=[("现配:两遍",[A2,trkpct(A2)],()),
     ("v7 替换两遍",[R5,trkpct(R5)],()),
     ("两遍+v7列",[A2,trkpct(A2),R5],()),
     ("三遍均值(含v7)",[A3,trkpct(A3)],())]
# 150 条重叠、同配置正面对撞
old={}
for l in open(Path.home()/"tutu-video-eval/data/apijudge/out/dev150_flash_v2.jsonl"):
    o=json.loads(l); r=o.get("result") or {}
...
if com:
    ...
    print(f"  bad_score 均值 历史 {a.mean():.1f} vs v7 {b.mean():.1f};秩相关 {spearmanr(a,b).statistic:.3f}")
    print(f"  AUC 历史 {auc(a,yy):.4f} vs v7 {auc(b,yy):.4f}")
print("\n=== 融合层 ===")
for nm,fc,ex in CFG:
