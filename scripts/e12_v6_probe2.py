...
def main():
    ...
    f = open(ROOT / "data/e12_v6_probe2.jsonl", "w")
    for p in probe:
        ...
        if len(jpgs) >= 4:
            ...
            row["eyebrow_zoom"] = j
        full = read_full_frames(ROOT / "data/corpus_videos" / p["rel"])
        if len(full) >= 4:
            j = parse_json(ask(full, QT)) or {"parse_error": 1}
            row["tail_full"] = j
    ...
    f.close()
    print("V6_PROBE2_DONE", flush=True)
